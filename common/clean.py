...
def bg_color(value, message=_('Invalid color, try #RRGGBB')):
  if not value:
    return value
...
def bg_image(value, message=_('Invalid background image path')):
...
def datetime(value, message=_('Invalid datetime')):
  return cleaner.datetime(value, message)

def nick(value, message=_('Invalid nick')):
...
def channel(value, message=_('Invalid channel name')):
...
def oauth_type(value, message=_('Invalid OAuth type')):
  if value not in ['web', 'desktop', 'mobile']:
    raise exception.ValidationError(message)
  return value

def user(value, message=_('Invalid nick')):
  """ expects to get a nick in one of the following forms:
...
def mobile(value):
  ...
  if not match:
    raise exception.ValidationError(_('Invalid mobile number'))
...
def url(value, message=_('Invalid url')):
